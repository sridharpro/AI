...
## additionally using stochastic search strategy
# Solution state:
#['C', 'A', 'T', '#']
#['O', 'N', '#', 'A']
#['A', 'D', '#', '#']
...
class Crossword:
    ...
    def probability_calc(self,L):
        
        prob=1
        for i in range(len(L)):
            if len(L[i])==len(self.word_batch[i]):
                prob=prob*max(random.random(),1-random.random())
            else:
                prob=prob*min(random.random(),1-random.random())
        return prob
    ...
    def solve(self):
        ...
        for x in combo_words:
            templist=[]
            templist.append(self.probability_calc(x))
